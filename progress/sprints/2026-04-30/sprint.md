# Sprint — generated 2026-04-30

> This is the **comprehensive sprint plan**, not today's work. It covers one step (`read`) for each of the four subjects at their current state position. Total: ~13 items, ~9–10 hours of work, sized for ~1 week of part-time effort.
>
> Don't try to do this in one day. **Run `/daily-tasks` to get a small chewable slice (2–3 items, 1–2 hours).** The items checklist is at [items.md](items.md). Re-run `/daily-tasks` in the same day if you finish — it'll ask and pull more.
>
> When every item is checked off, the sprint is complete and `/next-sprint` will generate the next one (advancing state by one step).

## Sprint overview

| Subject | Topic | Step | Items |
|---|---|---|---|
| Agentic workflows | `what-is-an-agent` (Lvl 1, #1) | read | S-01, S-02, S-03 |
| Software architecture | `separation-of-concerns` (Lvl 1, #1) | read | S-04, S-05, S-06 |
| Design patterns | Head First Ch 1 SimUDuck (Lvl 1, #1) | read | S-07, S-08, S-09 |
| DevOps | `Bicep modules + parameters` (Lvl 2, #1) | read | S-10, S-11, S-12, S-13 |
